 ...
 *     > 4
 */
public class ThreeSplitArchitectureTest {


    private static final String NODES_FILE = "src/test/resources/service_stages.csv";
    private static final String EDGES_FILE = "src/test/resources/three_split_architecture.csv";
    ...
    private void assertNode1(
        ...
        assertEquals(3, succStages.size());
        ...
        for (int i = 0; i < succStagesList.size(); i++) {
            if (HOST_2.equals(succStagesList.get(i).getServiceHost())) {
                stageTwoIndex = i;
            }
            else if (HOST_3.equals(succStagesList.get(i).getServiceHost())) {
                stageThreeIndex = i;
            }
        ...
        LinkInformation linkInformation = graph.edgeValue(
                stage,
                succStagesList.get(stageTwoIndex))
                .orElse(null);

        assertNotNull(linkInformation);
        assertEquals(VAR_2, linkInformation.getSourceFieldName());

        linkInformation = graph.edgeValue(stage, succStagesList.get(stageThreeIndex))
                .orElse(null);
