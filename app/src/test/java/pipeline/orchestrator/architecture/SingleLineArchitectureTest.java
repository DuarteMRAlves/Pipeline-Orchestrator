 ...
 * 1 > > 2 > > 3 > > 4
 */
public class SingleLineArchitectureTest {

    private static final String NODES_FILE = "src/test/resources/service_stages.csv";
    private static final String EDGES_FILE = "src/test/resources/line_architecture.csv";
    ...
    private void assertNode2(
        ...
        Set<StageInformation> predStages = graph.predecessors(stage);
        assertEquals(1, succStages.size());
        assertEquals(HOST_3, succStages.iterator().next().getServiceHost());
    ...
    private void assertNode4(
        ...
        assertEquals(1, predStages.size());
        assertEquals(HOST_3, predStages.iterator().next().getServiceHost());
    }
}
