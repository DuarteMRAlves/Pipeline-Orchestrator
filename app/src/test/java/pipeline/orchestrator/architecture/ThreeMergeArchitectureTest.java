 ...
 * 3 >
 */
public class ThreeMergeArchitectureTest {

    private static final String NODES_FILE = "src/test/resources/service_stages.csv";
    private static final String EDGES_FILE = "src/test/resources/three_merge_architecture.csv";
    ...
    private void assertNode4(
        ...
        assertEquals(3, predStages.size());

        List<StageInformation> predStagesList = Lists.newArrayList(predStages.iterator());
        assertEquals(3, predStagesList.size());

        int stageOneIndex = -1;
        int stageTwoIndex = -1;
        int stageThreeIndex = -1;
        for (int i = 0; i < predStagesList.size(); i++) {
            if (HOST_1.equals(predStagesList.get(i).getServiceHost())) {
                stageOneIndex = i;
            }
            else if (HOST_2.equals(predStagesList.get(i).getServiceHost())) {
                stageTwoIndex = i;
            }
        ...
        assertNotEquals(-1, stageOneIndex);
        assertNotEquals(-1, stageTwoIndex);
        assertNotEquals(-1, stageThreeIndex);

        LinkInformation linkInformation = graph.edgeValue(predStagesList.get(stageOneIndex), stage)
                .orElse(null);

        assertNotNull(linkInformation);
        assertEquals(VAR_1, linkInformation.getTargetFieldName());

        linkInformation = graph.edgeValue(predStagesList.get(stageTwoIndex), stage)
                .orElse(null);
